...
define(
    [
        ...
        'MW_Onestepcheckout/js/model/validate-shipping',
        'MW_Onestepcheckout/js/model/google-autocomplete-address'
    ],
    function (
        $,
        _,
        ...
        return Component.extend({
            defaults: {
                template: 'MW_Onestepcheckout/shipping'
            },
            ...
            initGoogleAddress: function(){
                if (window.checkoutConfig.suggest_address == true && window.checkoutConfig.google_api_key && window.checkoutConfig.validate_google_api_key ){
                    setTimeout(function(){
                        GoogleAutocompleteAddress.init('co-shipping-form','shipping');
                    },2000);
                }
                if (window.checkoutConfig.suggest_address == true && window.checkoutConfig.google_api_key && !window.checkoutConfig.validate_google_api_key ){
                    alert("The provided Google API key is invalid.");
                }
            }
            // bindDefaultRegion: function () {
            //     if(window.checkoutConfig.defaultRegion){
            //         var $region = $('#co-shipping-form').find('[name$="region"]');
            //         $region.eq(0).val(window.checkoutConfig.defaultRegion);
